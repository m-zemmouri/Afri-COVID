...
def check_file(file_name):
    ...
    try:
        ...
        if (
            ds.InstitutionName
            or ds.InstitutionAddress
            or ds.ReferringPhysicianName
            or ds.PhysiciansOfRecord
            or ds.PerformingPhysicianName
            or ds.PatientName
            or ds.RequestingPhysician
            or ds.PatientState
        ):
            log_info(f"{file_anonym} is badly cleaned")
            return
        log_info(f"{file_name} is ok")
        new_destination = Path(file_name.replace(archiv_path, archiv_ok)).parent
        Path(new_destination).mkdir(parents=True, exist_ok=True)
        shutil.move(file_name, new_destination)
        new_destination = Path(file_anonym.replace(anonym_path, anonym_ok)).parent
        Path(new_destination).mkdir(parents=True, exist_ok=True)
...
def hide_elements(dcm_dataset, file_name):
    try:
        ...
        dcm_dataset.InstitutionName = dcm_dataset.InstitutionAddress = (
            dcm_dataset.ReferringPhysicianName
        ) = dcm_dataset.PhysiciansOfRecord = dcm_dataset.PerformingPhysicianName = (
            dcm_dataset.PatientName
        ) = dcm_dataset.RequestingPhysician = dcm_dataset.PatientState = ""
...
def main():
    ...
    df_global.to_csv(f"{source_path} {now_string}_metadata.csv", index=False, sep=";")
    log_file.close()
