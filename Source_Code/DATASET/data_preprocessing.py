...
def group_data(delimiter):
    ...
    for filename in all_files:
        try:
            df_file = pd.read_csv(
                filename, index_col=None, header=0, sep=delimiter, dtype=columns_types
            )
    ...
    df.to_csv(
        f"F:/DataSet/2022-02-22/metadata/output/{file_type}.csv", index=False, sep=";"
    )
    return df
    # extract_data(df)
...
def extract_data(data_frame):
    ...
    patien_study_col = set(patien_col + list(study_set - patien_set))
    ...
    df_patien_study_serie.to_csv(
        path + r"\output\patien_study_serie_dataset.csv", index=False
    )
    # select serie cols
...
def merge_meta():
    delimiter = ";"
    df1 = pd.read_csv(
        "F:/DataSet/2022-09-24/output/grouped_dataset.csv",
        index_col="SOPInstanceUID",
        header=0,
        sep=delimiter,
        dtype=columns_types,
    )
    df2 = pd.read_csv(
        "I:/2 OK/Input/_metadata.csv",
        index_col="SOPInstanceUID",
        header=0,
        sep=delimiter,
        dtype=columns_types,
