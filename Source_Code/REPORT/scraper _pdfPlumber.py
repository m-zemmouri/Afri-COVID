    ...
    if file_name.endswith(".pdf"):
        ...
        try:
            ...
            txt_file = codecs.open(
                file_name.replace(".pdf", "_pdfplumber.txt"), "a", "utf-8"
            )
            ...
            # Conclusion
            x = re.search(
                ".*Conclusion.*", report_text, flags=re.IGNORECASE | re.DOTALL
            )
            if x:
                row["Conclusion"] = "True"
            else:
            ...
            # LesionRate
            x = re.findall(
                "([0-9]+)%.*parenchyme", report_text, flags=re.IGNORECASE | re.DOTALL
            )
